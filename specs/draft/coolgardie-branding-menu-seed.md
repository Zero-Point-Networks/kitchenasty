...
1. **Demo-only seed data** — `prisma/seed.ts` seeds a fictional San Francisco Mediterranean restaurant ("Saffron & Sage", `prisma/seed.ts:55`, `prisma/seed.ts:610`) with USD pricing, US address, and Unsplash imagery. The deployment target is the Coolgardie Gold Rush Motel in WA, Australia.
2. **No venue-specific seed path** — the only seed entry point is `packages/server/package.json:21` (`"seed": "tsx ../../prisma/seed.ts"`). There is no way to provision a production-ready Coolgardie database without hand-entering ~41 menu items, settings, and hours through the admin UI.
...
| `prisma/schema.prisma` | `SiteSettings` (line 586), `Location`, `Category` (188), `MenuItem` (212), `MenuOption` (246), `MenuItemAllergen` (319), `MenuItemMealtime` (300) |
...
const coolgardieSettings = {
  ...
  orderSettings: { enabled: true, dineInEnabled: true, enableTipping: false, taxRate: 0 }, // AU prices are GST-inclusive
...
- 10 `Table`s (6×4-seat, 3×6-seat, 1×2-seat — 44 seats, matching the venue's stated capacity of ~45) with `dev-table-1-qr` on Table 1, matching the demo-seed QR convention so the dine-in flow works out of the box.
- No `DeliveryZone`s.
...
Options are guarded with the demo seed's `menuOption.count()` pattern so re-seeding doesn't duplicate.
...
- [ ] **T1.3** Seed `Location` (slug `coolgardie`), operating hours 17:30–19:30 daily, Dinner mealtime, 10 tables (~45 seats, `dev-table-1-qr` on Table 1) `[server]` `[~60 LOC]` — depends: T1.1
